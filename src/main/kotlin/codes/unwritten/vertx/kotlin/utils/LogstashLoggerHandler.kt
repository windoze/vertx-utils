 ...
 */
class LogstashLoggerHandler(private val immediate: Boolean = false, private val format: LoggerFormat = DEFAULT_FORMAT) : LoggerHandler {
    private val logger = LoggerFactory.getLogger(this.javaClass)

    /** The Date formatter (UTC JS compatible format)
     */
    private val dateTimeFormat = Utils.createRFC1123DateTimeFormatter()

    private fun log(context: RoutingContext, timestamp: Long, remoteClient: String?, version: HttpVersion, method: HttpMethod, uri: String) {
        ...
        when (format) {
            ...
            LoggerFormat.SHORT -> message = String.format("%s - %s %s %s %d %d - %d ms",
                    remoteClient,
                    method,
                    uri,
                    versionFormatted,
            ...
            LoggerFormat.TINY -> message = String.format("%s %s %d %d - %d ms",
                    method,
                    uri,
                    status,
                    contentLength,
